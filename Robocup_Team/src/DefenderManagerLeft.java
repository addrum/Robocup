...
public class DefenderManagerLeft implements ControllerPlayer {
	...
	public void postInfo() {									//////////////////////////////////////////////////////////////////////////////////////////////////////////
		if(distBall < 20) { 									//if the ball is within a range of 20 from the agent (close)
			...
			if(canSeeCenter && distCenter < 55){				//|		if the agent can see the the center of the field and the distance to it is less than 55
				if(canSeePenaltyLeft && !canSeeBall) {			//|		|	if the agent can also see the left corner of his team's penalty box but can't see the ball
																//|		|	(meaning he is behind the penalty box, closer to the field end)
					getPlayer().turn(dirPenaltyLeft);			//|		|	|	then turn towards the left corner of his team's penalty box
					getPlayer().dash(randomDashValueVeryFast());//|		|	|	and dash very fast towards it
				} else if(distPenaltyLeft<1){					//|		|	if the agent is in range 1 to the left corner of his team's penalty box
					getPlayer().turn(90);						//|		|	|	then turn the agent 90 degrees clockwise from where he is currently facing
				}												//|		|	|
			} else if(canSeePenaltyLeft){						//|		if the agent can see the left corner of his own team's penalty box (but not the center of the field)
																//|		(also this is assuming the agent is at a greater distance than 55 from the center of the field)
				getPlayer().turn(dirPenaltyLeft);				//|		|	then turn towards the left corner of the agent's own team's penalty box
				getPlayer().dash(randomDashValueVeryFast());	//|		|	and dash very fast towards it
			} else if(distPenaltyLeft<1){						//|		if the agent is closer than 1 to the left corner of his own team's penalty box (assuming he can not
	...
	public void infoSeeFlagPenaltyOwn(Flag flag, double distance, double direction, double distChange, double dirChange, double bodyFacingDirection, double headFacingDirection) {
		//here we are saving data for when the agent sees the left corner of his own team's penalty box
